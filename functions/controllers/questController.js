...
exports.getQuestById = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      userStatus: progress ? progress.status : "not_started",
      totalXPEarned: progress ? progress.totalXPEarned : 0,
    });
  } catch (err) {
...
exports.completeTask = async (req, res) => {
  try {
    ...
    if (taskProgress && taskProgress.isCompleted) {
...
exports.getQuestDialogue = async (req, res) => {
  try {
    ...
    const activeTaskProgress = progress.taskProgress.find(
        (tp) => !tp.isCompleted,
    );
    const currentTask = activeTaskProgress ?
      quest.tasks.id(activeTaskProgress.taskId) : null;
...
exports.checkNearbyTriggers = async (req, res) => {
  try {
    ...
    // Find active quests that have trigger locations.
    const questsWithTriggers = await Quest.find({
      "isActive": true,
      "triggerLocation": {$exists: true},
      "triggerLocation.coordinates.lat": {$ne: null},
      "triggerLocation.coordinates.lng": {$ne: null},
    });

    // filter triggers based on user conditions
    const availableTriggers = [];

    for (const triggerQuest of questsWithTriggers) {
      // Existing logic keeps room for one-time trigger state if tracked later.
      const alreadyTriggered = false;
      ...
      const distance = calculateDistance(
          userLat,
          userLng,
          triggerQuest.triggerLocation.coordinates.lat,
          triggerQuest.triggerLocation.coordinates.lng,
      );
      ...
      availableTriggers.push({
        triggerId: triggerQuest._id,
        triggerType: "location",
        location: triggerQuest.triggerLocation,
        actions: [],
        distance,
      });
    }
...
/**
 * Award XP and recompute level and skill points in Firestore.
 * @param {string} userId Authenticated user id.
 * @param {number} xp XP to add.
 * @return {Promise<{leveledUp: boolean, newLevel: number}>}
 */
...
/**
 * Compute distance between two lat/lng points in meters.
...
/**
 * Validate a task submission payload by task type.
 * @param {Object} task Quest task document.
...
async function validateTaskAnswer(task, body) {
  switch (task.type) {
    ...
    case "number_input": {
      if (parseInt(body.answer, 10) !== task.numberInputData.correctAnswer) {
